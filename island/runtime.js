import {cleanLowerPark} from '/67park-codex-preview-20260914/app/park-lower-cleanup.js?v=20260914p';
import {repairEastRoadEnd} from '/67park-codex-preview-20260914/app/east-road-end.js?v=20260914-east1';
import {installLobbyCourts} from '../app/lobby-courts.js';
import {installIslandSwimBoundary} from '/67park-codex-preview-20260914/app/island-swim-runtime.js';
import {installParcelGapQA} from '/67park-codex-preview-20260914/app/parcel-gap-qa.js';
import {applyParcelPaving} from '/67park-codex-preview-20260914/app/island-parcel-paving.js';
import {installParcelCornerQA} from '/67park-codex-preview-20260914/app/parcel-corner-qa.js';
import {assetFetch} from '/67park-codex-preview-20260914/app/entry-loading.js';
const islandFetch=(u,...a)=>assetFetch(typeof u==='string'&&u.startsWith('./')?'/67park-codex-preview-20260914/island/'+u.slice(2):u,...a);
...
import {repairIslandStairs,stairAt,stairsRailBlocked} from '/67park-codex-preview-20260914/app/island-stair-geometry.js';
import {applyCurbJoins} from '/67park-codex-preview-20260914/app/island-curb-joins.js';
import {applyParkEdges} from '/67park-codex-preview-20260914/app/island-park-edges.js';
...
import {entryStage} from '/67park-codex-preview-20260914/app/entry-loading.js';
...
const GLB_DOSYASI=urlParams.get('model')==='pending'
  ...
  : '/67park-codex-preview-20260914/island/ada_calisma.glb';
...
await entryStage(13,'Finishing the northern neighbourhood');
  ...
  const parkEdgePatch=await islandFetch('/67park-codex-preview-20260914/repairs/park-edges-v4.json').then(r=>{if(!r.ok)throw Error('Park edge repair missing');return r.json();});
  renderer.domElement.dataset.parkEdges=JSON.stringify(applyParkEdges(kok,parkEdgePatch));
  const curbJoinPatch=await islandFetch('/67park-codex-preview-20260914/repairs/curb-joins-v3.json').then(r=>{if(!r.ok)throw Error('Curb join repair missing');return r.json();});
  renderer.domElement.dataset.curbJoins=JSON.stringify(applyCurbJoins(kok,curbJoinPatch));
  renderer.domElement.dataset.parcelPaving=JSON.stringify(applyParcelPaving(kok));repairEastRoadEnd(kok);cleanLowerPark(kok);
