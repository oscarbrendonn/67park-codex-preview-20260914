...
const [landformMeta,landformBuffer]=await Promise.all([
  islandFetch('./landform-v35.json').then(r=>r.json()),islandFetch('./landform-v35.bin').then(r=>r.arrayBuffer())]);
const [shoreRoundMeta,shoreRoundBuffer]=await Promise.all([
  islandFetch('./shore-round-v37.json').then(r=>r.json()),islandFetch('./shore-round-v37.bin').then(r=>r.arrayBuffer())]);
...
await entryStage(2,'Preparing land and paths');
  ...
  const smallIslandPropsReady=loadSmallIslandProps({scene:sahne,renderer,sample:zeminVurusu,variant:'codex'}).then(props=>{
...
await entryStage(3,'Preparing the park trees');
  parkProps57=await loadParkProps63({scene:sahne,renderer,sample:zeminVurusu,variant:'codex'});
...
await entryStage(13,'Finishing the northern neighbourhood');
  ...
  const parkEdgePatch=await islandFetch('/67park-codex-preview-20260914/repairs/park-edges.json').then(r=>{if(!r.ok)throw Error('Park edge repair missing');return r.json();});
  renderer.domElement.dataset.parkEdges=JSON.stringify(applyParkEdges(kok,parkEdgePatch));
  const curbJoinPatch=await islandFetch('/67park-codex-preview-20260914/repairs/curb-joins.json').then(r=>{if(!r.ok)throw Error('Curb join repair missing');return r.json();});
